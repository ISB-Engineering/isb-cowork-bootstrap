...
function Write-IsbRoleCatalogSource {
  param(
    ...
    [Parameter(Mandatory = $true)]
    [hashtable]$RoleSkillMap,

    [Parameter(Mandatory = $true)]
    [hashtable]$SkillDetails
  )
...
description: Use when an ISB employee asks what Claude skills are available for their role, what was installed for them, or asks to show the role skill catalog.
---

# ISB Role Skills Catalog

When a user asks what skills are available to them, inspect the role files in `references/roles/` and answer from those files.

If the user names a specific role, open that role file. If they ask generally, summarize `references/installed-roles.md` first, then offer the relevant role files.

Do not invent skills that are not listed in the local role files.
'@
  ...
  foreach ($role in ($Roles | Sort-Object -Unique)) {
    if (-not $RoleSkillMap.ContainsKey($role)) { continue }
    $installedRoles.Add($role) | Out-Null

    $lines = New-Object System.Collections.Generic.List[string]
    $lines.Add("# Role: $role") | Out-Null
    $lines.Add("") | Out-Null
    $lines.Add("Installed Claude skills for this role on this computer.") | Out-Null
    $lines.Add("") | Out-Null

    foreach ($skillName in ($RoleSkillMap[$role] | Sort-Object -Unique)) {
      $allSkills.Add($skillName) | Out-Null
      $detail = $null
      if ($SkillDetails.ContainsKey($skillName)) {
        $detail = $SkillDetails[$skillName]
      }
      ...
      $lines.Add("- ``$skillName`` ($owner): $description") | Out-Null
    }

    Set-Content -LiteralPath (Join-Path $rolesDir "$role.md") -Value ($lines -join [Environment]::NewLine) -Encoding UTF8
  }

  $summary = New-Object System.Collections.Generic.List[string]
  $summary.Add("# Installed ISB Role Skills") | Out-Null
  $summary.Add("") | Out-Null
  $summary.Add("This file was generated by isb-cowork-bootstrap for the roles selected during installation.") | Out-Null
  $summary.Add("") | Out-Null
  $summary.Add("Roles: $($installedRoles -join ', ')") | Out-Null
  $summary.Add("Unique skills: $($allSkills.Count)") | Out-Null
  $summary.Add("") | Out-Null
  $summary.Add("Role files:") | Out-Null
